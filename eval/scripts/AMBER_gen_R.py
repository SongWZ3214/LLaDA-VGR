...
PRETRAINED = str(REPO_ROOT / "train" / "exp" / "llada_v_lora_rank64_1227")
...
IMAGE_DIR = str(Path(os.environ.get("AMBER_IMAGE_DIR", str(WORKSPACE_ROOT / "exp" / "AMBER" / "data" / "image"))))
BASE_QUERY_JSON = str(Path(os.environ.get("AMBER_QUERY_JSON", str(WORKSPACE_ROOT / "exp" / "AMBER" / "data" / "query" / "query_generative.json"))))
...
if dino_thresh is not None:
    # 如果存在环境变量，则加上阈值后缀
    QUERY_JSON = BASE_QUERY_JSON.replace(".json", f"-thresh_0330{dino_thresh}.json")
else:
    # 否则使用原名
    QUERY_JSON = BASE_QUERY_JSON.replace(".json", f"_0330_vgr.json")
# ==================== 修改结束 ======================================
START_INDEX = 0  # 起始索引（包含），设置为None表示从0开始
END_INDEX = 50  # 终止索引（不包含），设置为None表示处理到末尾
...
def process_single_sample(refinement_engine: RefinementEngine, image_data, prompt_text, device: str = "cuda:0"):
    """
    ...
    try:
        ...
        # ==================== 修改：与 Baseline 对齐的监控起点 ====================
        torch_device = torch.device(device)
        if torch.cuda.is_available():
            torch.cuda.synchronize(torch_device)
            torch.cuda.empty_cache()  # 【关键新增】彻底清空 Pytorch 缓存碎片
        ...
        final_response, metadata = refinement_engine.refine(
            ...
            # 【强制对齐官方 Demo 的生成参数】
            steps=GEN_STEPS,  # 对齐 demo 的 steps=128
            gen_length=GEN_LENGTH,  # 对齐 demo 的 gen_length=128
            block_length=BLOCK_LENGTH,  # 对齐 demo 的 block_length=128
            prefix_refresh_interval=PREFIX_REFRESH_INTERVAL,  # =32
            threshold=THRESHOLD  # =1
        )

        # ==================== 修改：与 Baseline 对齐的监控终点 ====================
        if torch.cuda.is_available():
            torch.cuda.synchronize(torch_device)  # 【新增】等待 DINO 和模型彻底算完
        ...
        # 修改：增加安全检查，防止 logger 为 None 时崩溃
        if 'logger' in globals() and globals()['logger'] is not None:
            logger.error(f"Error during refinement: {e}")
            import traceback
            logger.error(traceback.format_exc())
        else:
            # 退化到 print，确保我们能在终端看到真实的报错
            print(f"Error during refinement (Logger missing): {e}")
            import traceback
            traceback.print_exc()
        return {'error': str(e)}
    finally:
...
def process_data_chunk(
        gpu_id: int,
        tasks: List[Tuple[int, str, str, Path]],  # List of (entry_id, image_filename, query_text, image_path)
        query_json: str,
        result_queue: Queue,
        log_file: str,
        json_lock: mp.Lock
    ...
    Args:
        gpu_id: GPU ID (0-3)
        tasks: 要处理的任务列表 [(entry_id, image_filename, query_text, image_path), ...]
    ...
    # ================= 关键修改：隔离显卡可见性，与 Baseline 保持完全一致 =================
    import os
    os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu_id)
    device = "cuda:0"  # 锁定为 cuda:0
    # ============================================================================
    ...
    try:
        ...
        for entry_id, image_filename, query_text, image_path in tqdm(tasks, desc=f"GPU {gpu_id} 处理任务"):
            try:
                ...
                # 使用query_text作为prompt，如果为空则使用默认prompt
                    ...
                    with json_lock:
                        try:
                            ...
                            for entry in query_data:
                                if entry.get('id') == entry_id:
                                    entry['response'] = None
                                    # 💥 如果 result 是 None，这里的 'error' in result 会引发 TypeError!
                ...
                initial_response = result.get('initial_response', '')  # 提取初始输出
                latency = result.get('latency_seconds', 0.0)  # 提取延迟
                ...
                with json_lock:
                    try:
                        ...
                        process_logger.debug(
                            f"GPU {gpu_id}: 成功更新JSON文件，entry_id: {entry_id}, image: {image_filename}")
                    except Exception as e:
                        process_logger.error(f"GPU {gpu_id}: 更新JSON文件失败: {e}")
                        import traceback
                        process_logger.error(traceback.format_exc())
...
def reprocess_failed_samples(
    ...
    for idx, (sample_index, old_data, filepath) in enumerate(tqdm(failed_samples, desc="重新处理失败样本")):
        try:
            ...
            result = process_single_sample(
                refinement_engine,
                image_data, DEFAULT_PROMPT_TEXT
            )
...
def main():
    ...
    base_json_path = Path(BASE_QUERY_JSON)  # <--- 【重点：补上这一行！】
    query_json_path.parent.mkdir(parents=True, exist_ok=True)

    # ==================== 修改开始：自动复制初始数据 ====================
    import shutil
    # 如果带后缀的新文件不存在，但基础文件存在，则复制一份作为起点
    ...
    for entry in query_data:
        ...
        if entry_id is None:
            logger.warning(f"警告: 条目缺少id字段，跳过: {entry}")
            continue

        if not image_filename:
            logger.warning(f"警告: 条目 {entry_id} 缺少image字段，跳过")
            continue

        # 检查是否已经生成过（如果已有response且不为空，则跳过）
