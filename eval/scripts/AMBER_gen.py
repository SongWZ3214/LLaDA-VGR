...
TOKEN_SELECTION_MODE = "confidence"  # Token 选择模式: "jitter"（选择jitter最高的token）、"random"（随机选择一个token）、"confidence"（选择confidence最低的token）

# AMBER 数据集路径和输出路径
IMAGE_DIR = "/data0/swz/exp/AMBER/data/image"  # 图片文件目录
QUERY_JSON = "/data0/swz/exp/AMBER/data/query/llada-vgr-0105-confidence.json"  # 输入/输出JSON文件
            ...
            try:
                ...
                with json_lock:
                    try:
                        ...
                        with open(query_json, 'w', encoding='utf-8') as f:
                            json.dump(query_data, f, ensure_ascii=False, indent=2)
                        process_logger.debug(f"GPU {gpu_id}: 成功更新JSON文件，entry_id: {entry_id}, image: {image_filename}")
                    except Exception as e:
                        process_logger.error(f"GPU {gpu_id}: 更新JSON文件失败: {e}")
                        import traceback
                        process_logger.error(traceback.format_exc())
